...
function myplugin_custom_login_styles() {
	...
	if ( 'enable' === $styles ) {
		...
		*/
		wp_enqueue_script( 'confetti-animation', plugin_dir_url( dirname( __FILE__ ) ) . 'public/js/confetti-falling-animation/confetti.js', array(), null, true);
		
		wp_enqueue_style( 'myplugin', plugin_dir_url( dirname( __FILE__ ) ) . 'public/css/style-login.css', array(), null, 'screen' );

		
		wp_enqueue_script( 'myplugin', plugin_dir_url( dirname( __FILE__ ) ) . 'public/js/script-login.js', array(), null, true );
